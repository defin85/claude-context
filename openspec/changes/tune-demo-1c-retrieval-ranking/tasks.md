## 1. Baseline And Evaluation Harness
...
- [ ] 1.6 Validate every expected path prefix against the indexed `examples/demo-1c` fixture before treating Hit@10 24/30 as a hard gate; fix stale labels in the dataset or document unreachable/ambiguous query IDs in the report.
...
- [ ] 3.4 Implement deterministic ranking signals for object-kind, object-name, and form/command intent matches.
- [ ] 3.5 Keep unknown or non-1C paths neutral so generic code search behavior does not fail on unfamiliar layouts.
...
- [ ] 4.2 Implement soft duplicate control or diversity reranking after fusion while preserving legitimate multi-chunk hits.
- [ ] 4.3 Rebalance fusion weights so broad lexical matches do not dominate specific path/object matches.
...
- [ ] 6.4 Confirm the tuned run improves over Hit@10 18/30 and reaches Hit@10 24/30 or documents any lower accepted threshold with query-level justification.
...
- [ ] 7.2 Summarize ranking changes, baseline-vs-tuned metrics, remaining ambiguous labels, and any known regressions.
